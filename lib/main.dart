...
import 'dart:convert';
//import 'dart:html';
import 'package:billing_app_new1/PaymentResponseNew.dart';
import 'package:billing_app_new1/SalesRequest.dart';
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  PaymentResponseNew paymentresponsenew;
  ...
  Future<void> _receiveFromHost(MethodCall call) async {
    try {
      ...
      paymentresponsenew=PaymentResponseNew.fromJson(response);
        // Map<String, dynamic> response =
        //     Map<String, dynamic>.from(call.arguments);
        // PaymentResponse paymentResponseModel =
        //     PaymentResponse.fromJson(response);
    //    print("PaymentResponse object: ${paymentResponseModel}");
         print("PaymentResponse object: ${paymentresponsenew}");
        ...
        var r = PaymentResponseNew.fromJson(response);
      ...
      //  savePaymentResponseAPI(context, paymentResponseModel.toJson());
        savePaymentResponseAPINew(context, paymentresponsenew.toJson());
  ...
  savePaymentResponseAPI(BuildContext context, passJson) async {
    ...
    savePaymentResponseAPINew(BuildContext context, passJson) async {
    // AppUtils.showProgressDialog(context, "Please Wait...");
    var vStatus = await ApiInterface().savePaymentResponseNew(passJson);
    if ((vStatus.toString() != 'null')) {
      // If the call to the server was successful, parse the JSON
      Future.delayed(Duration(milliseconds: 1)).then((value) async {
        List<dynamic> list = [json.decode(vStatus)];
